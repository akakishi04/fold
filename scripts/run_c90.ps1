...
"=== FOLD C90 V5-D router hidden-width frontier ===" | Set-Content $log -Encoding UTF8
Set-Location $repo
...
"commit = $head",
"stage = V5-D",
"diagnosis = supervised router hidden-width frontier",
"fresh_seeds = 20261131,20261132,20261133",
"hidden_widths = 2,4,8,16,32",
"selection = smallest quality-passing hidden width",
"maximum_selected_bytes_over_default = 0.50",
""
...
if ($resultBefore -ne $expectedResult -or $fixtureBefore -ne $expectedFixture) { throw "Protected artifact mismatch before C90" }

$c89 = Get-ChildItem "$repo\runs" -Directory -Filter "c89-v5d-runtime-crossover-*" | Sort-Object LastWriteTime -Descending | Where-Object { Test-Path (Join-Path $_.FullName "summary.json") } | Select-Object -First 1
if ($null -eq $c89) { throw "C89 summary not found" }
$c89Summary = Join-Path $c89.FullName "summary.json"

"=== import preflight ===" | Tee-Object -FilePath $log -Append
& $py -c "import fold_lm.v05_benchmarks.gate_d_router_hidden_width_frontier as m; print('C90 import OK:', m.EXPERIMENT_ID, m.SEEDS, m.HIDDEN_WIDTHS)" 2>&1 | Tee-Object -FilePath $log -Append
if ($LASTEXITCODE -ne 0) { throw "C90 import failed" }
...
if ($LASTEXITCODE -ne 0) { throw "C90 focused regression failed" }

$outDir = "$repo\runs\c90-v5d-router-frontier-$([guid]::NewGuid().ToString('N'))"
@("", "output_directory = $outDir", "C37_result_sha256_before = $resultBefore", "fixture_sha256_before = $fixtureBefore", "=== C90 benchmark ===") | Tee-Object -FilePath $log -Append

& $py -u -m fold_lm.v05_benchmarks.gate_d_router_hidden_width_frontier --c89-summary $c89Summary --output-dir $outDir 2>&1 | Tee-Object -FilePath $log -Append
$code = $LASTEXITCODE
if ($code -ne 0) { throw "C90 benchmark failed: $code" }
...
"", "=== C90 SUMMARY ===",
"status = $($s.status)",
"router_capacity_gate_passed = $($s.summary.router_capacity_gate_passed)",
"selected_hidden_width = $($s.summary.selected_hidden_width)",
"selected_bytes_over_default = $($s.summary.selected_bytes_over_default)",
