...
長文実行では、**初回入力は新規部分を一度処理し、継続turnではcommit済みhistoryをraw promptとして再prefillしない**persistent sessionを正規形にします。decode時は固定local contextとbounded memory readsだけをactive working setにし、履歴総長に比例するfull replay/full scanを標準経路から外します。

Visionでは、画像全体を巨大なpatch列のまま重いCoreへ渡さず、**Vision Encoder → bounded Visual Slots → Working State**を正規候補にします。元画像はRaw Visual Evidenceとして別保持し、細部が必要な場合は`OBSERVE(region, resolution)`で選択的に再観測します。同じ画像はpersistent visual cacheで再利用し、Visionをinformation acquisitionの一部として扱います。
...
| [long-context-runtime-v0.5.md](docs/long-context-runtime-v0.5.md) | 長文速度の正規契約。persistent session、prefix再処理禁止、bounded memory read、TTFT/decode scaling |
| [vision-runtime-v0.5.md](docs/vision-runtime-v0.5.md) | Vision正規設計。Frozen Encoder導入、Visual Resampler、raw evidence、region再観測、visual cache、Gate H |
...
最終方針はv0.5を優先します。v0.5を実装する場合は、`theory-v0.5.md`で数理境界、`long-context-runtime-v0.5.md`で長文sessionの実行境界、Vision作業では`vision-runtime-v0.5.md`で視覚証拠と再観測の境界を確認し、`development-roadmap-v0.5.md`で対象段階とGateを決めてから作業します。v0.4の比較条件を、全方式をゼロから共同学習する義務や、最終構造を固定する規則として再適用しません。FOLD-Rの数値保証・scope・訂正の意味論は維持します。
...
| v0.5 / 理論 / 長文runtime / Vision / ロードマップ / 不足情報取得 | 設計文書。モデル本体の実装・学習・性能検証とは別 |
