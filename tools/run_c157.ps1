...
try {
    Write-Output "=== FOLD C157 V5-E learned Controller readback bridge ==="
    ...
    Write-Output "stage = V5-E-LEARNED-CONTROLLER-READBACK-BRIDGE"
    Write-Output "source_requests = 82944; recorded_views = 331776; availability_modes = 2"
    Write-Output "fresh_router_seeds = 20261741,20261742,20261743"
    Write-Output "router_training_steps_each = 900; training_device = CPU"
    Write-Output "decisions = 1990656; distinct_router_input_patterns = 6"
    Write-Output "Controller = True; ANSWER_action = True; answer_generation = False"
    Write-Output "live_reobservation = False; retrieval = False; action_execution = False"
    Write-Output "expected_focused_tests = 440"
    ...
    Assert-NativeExit "C157 focused regression"
    Write-Output "=== C157 controller bridge ==="
    $out = Join-Path "runs" ("c157-v5e-controller-bridge-" + [guid]::NewGuid().ToString("N"))
    & $python -u -m fold_lm.v05_benchmarks.gate_e_c157_cli --c156-summary $C156Summary --output-dir $out
    Assert-NativeExit "C157 integration execution"
    $summary = Join-Path $out "summary.json"
    $report = Get-Content -LiteralPath $summary -Raw -Encoding UTF8 | ConvertFrom-Json
    if ($report.experiment_id -ne "C157-v5e-learned-controller-readback-bridge" -or $report.status -notin @("PASS","FAIL")) { throw "Wrong experiment/result" }
    ...
    if ($report.summary.learned_controller_exercised -ne $true -or $report.summary.answer_action_selection_exercised -ne $true -or $report.summary.answer_generation_exercised -ne $false -or $report.summary.retrieval_exercised -ne $false -or $report.summary.action_execution_exercised -ne $false) { throw "Unexpected execution scope" }
    if (($report.status -eq "PASS") -ne ($report.summary.controller_bridge_gate_passed -eq $true)) { throw "Status/gate mismatch" }
...
if (-not $postcheckOk) { throw "C157 postcheck failed; do not accept result" }
