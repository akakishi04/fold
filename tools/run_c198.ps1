...
$ParentPaths=@(
    $C197Summary,$C196Summary,$C195Summary,$C194Summary,$C193Summary,$C192Summary,$C191Summary,
    ...
    $C176Summary,$C174Summary
) | ForEach-Object {(Resolve-Path -LiteralPath $_).Path}
$Names=@(
    "c197","c196","c195","c194","c193","c192","c191","c190","c189","c188","c187","c186",
...
Write-Output "=== C198 authoring syntax preflight ==="
& $Python -m py_compile (Join-Path $Root "fold_lm\v05_benchmarks\gate_e_c198_stale_reservation_generic_loop.py") (Join-Path $Root "tests_lm\test_v05_c198_stale_reservation_generic_loop.py")
if($LASTEXITCODE -ne 0){throw "C198 Python syntax preflight failed"}
Write-Output "python_syntax_preflight = PASS"

Write-Output "=== FOLD C198 V5-E stale-reservation generic-loop containment ==="
Write-Output "repository = akakishi04/fold; branch = feat/sft-target-loss; commit = $ExpectedHead"
Write-Output "changed condition = successful reservation then trusted evidence identity refresh before dispatch"
Write-Output "arms = ALLOWED exact C197 replay + STALE_RESERVATION_AFTER_RESERVATION"
Write-Output "expected_focused_tests = 1677 (1653 existing +24 new); Gate_E = NOT_PASSED"

$Precheck=@'
from pathlib import Path
import sys
from fold_lm.v05_benchmarks import gate_e_c198_stale_reservation_generic_loop as b
b.precheck(Path(sys.argv[1]),*(Path(x) for x in sys.argv[2:24]),Path.cwd())
print('source_and_artifact_precheck = PASS; accepted C197 lineage fixed',flush=True)
'@
& $Python -u -c $Precheck @ParentPaths
if($LASTEXITCODE -ne 0){throw "C198 source/artifact precheck failed"}
...
assert suite.countTestCases()==1677,f'Expected1677 tests, got{suite.countTestCases()}'
r=unittest.TextTestRunner(verbosity=2).run(suite)
sys.exit(0 if r.wasSuccessful() else 1)
'@

$Out=Join-Path $Root ("runs\c198-v5e-stale-reservation-" + [guid]::NewGuid().ToString("N"))
Write-Output "output_dir = $Out"
$Completed=$false
try{
    Write-Output "=== focused regression ==="
    & $Python -u -c $Regression
    if($LASTEXITCODE -ne 0){throw "C198 regression failed; do not run benchmark"}
    Confirm-Repository

    $Cli=@("--output-dir",$Out,"--expected-head",$ExpectedHead)
    for($i=0;$i -lt $ParentPaths.Count;$i++){$Cli+=@("--$($Names[$i])-summary",$ParentPaths[$i])}
    & $Python -u -m fold_lm.v05_benchmarks.gate_e_c198_stale_reservation_generic_loop @Cli
    if($LASTEXITCODE -ne 0){throw "C198 execution failed; preserve invalid.json"}
...
out=Path(sys.argv[1])
b.precheck(Path(sys.argv[2]),*(Path(x) for x in sys.argv[3:25]),Path.cwd())
p=audit.read_json(out/'summary.json');b.validate_result(p)
assert p['commit_sha']==sys.argv[25]
...
print('stale_blocks =',len(p['stale_records']))
print('stale_rejections =',sum(r['stale_attempts'] for r in p['stale_records']))
print('stale_provider_calls =',sum(r['provider_calls'] for r in p['stale_records']))
print('stale_provider_reads =',sum(r['actual_provider_reads'] for r in p['stale_records']))
print('stale_publications =',sum(r['publications'] for r in p['stale_records']))
print('stale_retries =',sum(r['retries'] for r in p['stale_records']))
print('candidate_gate_passed =',b.gate(p['allowed_records'],p['stale_records']))
'@
    & $Python -u -c $Postcheck $Out @ParentPaths $ExpectedHead
    if($LASTEXITCODE -ne 0){throw "C198 artifact postcheck failed"}
    $Completed=$true
}
finally{
    Write-Output "=== C198 POSTCHECK ==="
    Confirm-Repository
