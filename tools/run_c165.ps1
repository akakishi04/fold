param(
    [Parameter(Mandatory = $true)][string]$C164Summary,
...
try {
    Write-Output "=== FOLD C165 V5-E live query permission denial ==="
    ...
    $required = @{}
    $required[$C164Summary] = "42DF47FE4C03D24DF064FF47EECF5B3C84B8DF2F36D6A47B7F861AC14778FED2"
    ...
    Write-Output "stage = V5-E-LIVE-QUERY-PERMISSION-DENIED"
    ...
    Write-Output "ANSWERED = 82944 expected; UNRESOLVED/PERMISSION_DENIED = 82944 expected"
    Write-Output "denied_branch_adapter_calls = 0; denied_branch_acquisition_budget_spend = 0"
    Write-Output "native_control_calls = 165888; adapted_output_calls = 165888; guard_calls = 768"
    Write-Output "training = 0; fresh_seeds = 0; expected_focused_tests = 665"
    Write-Output "changed_variable = runtime Permission.allowed true versus false ONLY"
    ...
    if ($count -ne "665") { throw "Expected 665 focused tests, got $count" }
    & $python -m unittest @tests -v
    Assert-NativeExit "C165 focused regression"
    Write-Output "=== C165 permission-only live comparison ==="
    $out = Join-Path "runs" ("c165-v5e-live-permission-denied-" + [guid]::NewGuid().ToString("N"))
    & $python -u -m fold_lm.v05_benchmarks.gate_e_c165_live_permission_denied `
        --c164-summary $C164Summary --output-dir $out
    Assert-NativeExit "C165 integration execution"
    $summary = Join-Path $out "summary.json"
    $report = Get-Content -LiteralPath $summary -Raw -Encoding UTF8 | ConvertFrom-Json
    if ($report.experiment_id -ne "C165-v5e-live-query-permission-denied" -or $report.stage -ne "V5-E-LIVE-QUERY-PERMISSION-DENIED" -or $report.status -notin @("PASS", "FAIL")) { throw "Wrong experiment/result" }
    ...
    $s = $report.summary
    if ($s.training_steps -ne 0 -or $s.fresh_seed_count -ne 0 -or $s.live_query_selection -ne $true -or $s.live_cycle_exercised -ne $true -or $s.structured_result_exercised -ne $true -or $s.permission_gate_exercised -ne $true -or $s.answer_generation_exercised -ne $false -or $s.production_state_commit -ne $false -or $s.new_evidence_epoch -ne $false) { throw "Unexpected scope" }
    if (($report.status -eq "PASS") -ne ($s.permission_denied_gate_passed -eq $true)) { throw "Status/gate mismatch" }
...
if (-not $postcheckOk) { throw "C165 postcheck failed; do not accept result" }
