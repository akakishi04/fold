...
if (-not $activeMatch.Success -or $activeMatch.Groups["id"].Value -ne "212") {
    $active = if ($activeMatch.Success) { "C" + $activeMatch.Groups["id"].Value } else { "UNRESOLVED" }
    Skip-Invocation -Reason "STALE_EXPERIMENT" -Detail "requested=C212 active=$active"
    return
}

$runnerPath = Join-Path $Root "tools\run_c212.ps1"
$runnerTokens = $null
...
try {
    ...
    try {
        .\tools\publish_experiment_log.ps1 -ExperimentId C212 -LogPath $log -ExecutionHead $ExpectedHead
    }
    catch {
        if ($null -ne $failure) {
            throw "C212 execution failed and log publication also failed. Execution error: $($failure.Exception.Message); publish error: $($_.Exception.Message)"
        }
        throw
    }
}
