...
if ($active.Count -ne 1 -or $active[0].Groups["id"].Value -ne "221") { Skip-Invocation "STALE_EXPERIMENT"; return }
$runnerPath = Join-Path $Root "tools\run_c221.ps1"
$runnerTokens = $null
...
$runArgs = @{
    ...
    C220Summary = (Join-Path $Root "runs\c220-v5f-frozen-learned-stack-5bc9d133c96348de81a24d19b3ef6f93\summary.json")
}
$failure = $null
try {
    & {
        Write-Output "=== C221 repository preflight ==="
        Write-Output "execution_head = $headNow"
        .\tools\run_c221.ps1 @runArgs
    } *>&1 | Tee-Object -FilePath $log
}
catch { $failure = $_ }
finally {
    try { .\tools\publish_experiment_log.ps1 -ExperimentId C221 -LogPath $log -ExecutionHead $ExpectedHead }
    catch {
        if ($null -ne $failure) { throw "C221 execution and publication failed: $($failure.Exception.Message); $($_.Exception.Message)" }
        throw
    }
}
