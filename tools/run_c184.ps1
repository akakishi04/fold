...
$ParentPaths = @($C183Summary, $C182Summary, $C181Summary, $C180Summary, $C179Summary,
    $C178Summary, $C177Summary, $C176Summary, $C174Summary) | ForEach-Object { (Resolve-Path -LiteralPath $_).Path }
$Hashes = @(
...
for ($i = 0; $i -lt 9; $i++) { $Protected += @{Path = $ParentPaths[$i]; Hash = $Hashes[$i]} }
...
Confirm-Repository
Write-Output "=== FOLD C184 V5-E canonical LOCAL fact-index adapter ==="
Write-Output "repository = akakishi04/fold; branch = feat/sft-target-loss; commit = $ExpectedHead"
Write-Output "all six frozen C181 checkpoints; source seeds181001/2/3; no training or fresh seeds"
Write-Output "handwritten first-occurrence normalization, NOT learned naming invariance; inverse slot map retained"
Write-Output "267948 normalization rows; 311040 source replay plus1296648 normalized predictions"
Write-Output "expected_focused_tests = 1249 (1217 existing + 32 new); Gate_E = NOT_PASSED"
$Precheck = @'
from pathlib import Path
import sys
from fold_lm.v05_benchmarks import gate_e_c184_canonical_fact_indices as b
b.precheck(*(Path(x) for x in sys.argv[1:10]),Path.cwd())
print('source_and_artifact_precheck = PASS',flush=True)
'@
& $Python -u -c $Precheck @ParentPaths
if ($LASTEXITCODE -ne 0) { throw "C184 source/artifact precheck failed" }
$Regression = @'
from pathlib import Path
import sys,unittest
from fold_lm.v05_benchmarks import gate_e_c184_canonical_fact_indices as b
...
assert suite.countTestCases()==1249,f'Expected1249 tests, got{suite.countTestCases()}'
r=unittest.TextTestRunner(verbosity=2).run(suite)
sys.exit(0 if r.wasSuccessful() else 1)
'@
$Out = Join-Path $Root ("runs\c184-v5e-canonical-indices-" + [guid]::NewGuid().ToString("N"))
$Completed = $false
try {
    Write-Output "=== focused regression ==="
    & $Python -u -c $Regression
    if ($LASTEXITCODE -ne 0) { throw "C184 regression failed; do not run benchmark" }
    Confirm-Repository
    & $Python -u -m fold_lm.v05_benchmarks.gate_e_c184_canonical_fact_indices `
        --c183-summary $ParentPaths[0] --c182-summary $ParentPaths[1] --c181-summary $ParentPaths[2] `
        --c180-summary $ParentPaths[3] --c179-summary $ParentPaths[4] --c178-summary $ParentPaths[5] `
        --c177-summary $ParentPaths[6] --c176-summary $ParentPaths[7] --c174-summary $ParentPaths[8] `
        --output-dir $Out --expected-head $ExpectedHead
    if ($LASTEXITCODE -ne 0) { throw "C184 execution failed; preserve invalid.json" }
    $Postcheck = @'
from pathlib import Path
import sys
from fold_lm.v05_benchmarks import gate_e_c184_canonical_fact_indices as b
from fold_lm.v05_benchmarks import gate_e_c175_frozen_prediction_audit as audit
out=Path(sys.argv[1]);b.precheck(*(Path(x) for x in sys.argv[2:11]),Path.cwd())
p=audit.read_json(out/'summary.json');b.validate_result(p)
assert p['commit_sha']==sys.argv[11]
assert sum(c['rows'] for c in p['normalization_checks'])==267948
...
print('candidate_errors =',sum(r['errors'] for r in p['records'] if r['arm']=='INTERNAL_SEMANTICS'))
'@
    & $Python -u -c $Postcheck $Out @ParentPaths $ExpectedHead
    if ($LASTEXITCODE -ne 0) { throw "C184 artifact postcheck failed" }
    $Completed = $true
}
finally {
    Write-Output "=== C184 POSTCHECK ==="
    Confirm-Repository
