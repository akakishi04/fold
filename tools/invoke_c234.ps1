...
if ($active.Count -ne 1 -or $active[0].Groups["id"].Value -ne "234") { Skip-Invocation "STALE_EXPERIMENT"; return }
$runnerPath = Join-Path $Root "tools\run_c234.ps1"
$runnerTokens = $null
...
$runArgs = @{
    ...
    C233Summary = (Join-Path $Root "runs\c233-v5b-core-ablation-f73f95f4afa44668a64b953f1bbad723\summary.json")
}
$failure = $null
try {
    & {
        Write-Output "=== C234 repository preflight ==="
        Write-Output "execution_head = $headNow"
        .\tools\run_c234.ps1 @runArgs
    } *>&1 | Tee-Object -FilePath $log
}
catch { $failure = $_ }
finally {
    try { .\tools\publish_experiment_log.ps1 -ExperimentId C234 -LogPath $log -ExecutionHead $ExpectedHead }
    catch {
        if ($null -ne $failure) { throw "C234 execution and publication failed: $($failure.Exception.Message); $($_.Exception.Message)" }
        throw
    }
}
