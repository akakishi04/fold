...
if ($active.Count -ne 1 -or $active[0].Groups["id"].Value -ne "238") { Skip-Invocation "STALE_EXPERIMENT"; return }
$runnerPath = Join-Path $Root "tools\run_c238.ps1"
$runnerTokens = $null
...
$runArgs = @{
    ...
    C237Summary = (Join-Path $Root "runs\c237-v5b-frozen-signal-audit-8dbdc04fdad84f49b41a78388ad0b7fb\summary.json")
    C236Summary = (Join-Path $Root "runs\c236-v5b-minimal-binding-9cc3975f1c304ea893b8b53104ee8f71\summary.json")
}
$failure = $null
try {
    & {
        Write-Output "=== C238 repository preflight ==="
        Write-Output "execution_head = $headNow"
        .\tools\run_c238.ps1 @runArgs
    } *>&1 | Tee-Object -FilePath $log
}
catch { $failure = $_ }
finally {
    try { .\tools\publish_experiment_log.ps1 -ExperimentId C238 -LogPath $log -ExecutionHead $ExpectedHead }
    catch {
        if ($null -ne $failure) { throw "C238 execution and publication failed: $($failure.Exception.Message); $($_.Exception.Message)" }
        throw
    }
}
