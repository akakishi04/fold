...
if ($active.Count -ne 1 -or $active[0].Groups["id"].Value -ne "224") { Skip-Invocation "STALE_EXPERIMENT"; return }
$runnerPath = Join-Path $Root "tools\run_c224.ps1"
$runnerTokens = $null
...
$runArgs = @{
    ...
    C223Summary = (Join-Path $Root "runs\c223-v5f-request-freshness-56cfd6d212764ccf98e7789aa964de52\summary.json")
}
$failure = $null
try {
    & {
        Write-Output "=== C224 repository preflight ==="
        Write-Output "execution_head = $headNow"
        .\tools\run_c224.ps1 @runArgs
    } *>&1 | Tee-Object -FilePath $log
}
catch { $failure = $_ }
finally {
    try { .\tools\publish_experiment_log.ps1 -ExperimentId C224 -LogPath $log -ExecutionHead $ExpectedHead }
    catch {
        if ($null -ne $failure) { throw "C224 execution and publication failed: $($failure.Exception.Message); $($_.Exception.Message)" }
        throw
    }
}
