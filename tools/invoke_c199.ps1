...
$runArgs=@{
 ...
 C198Summary=(Join-Path $Root "runs\c198-v5e-stale-reservation-c8dcde8fca5c4cc2979475d64b7c2fda\summary.json")
 C197Summary=(Join-Path $Root "runs\c197-v5e-provider-failure-reason-952131896bd7469296cda4621436c043\summary.json")
 C196Summary=(Join-Path $Root "runs\c196-v5e-result-aware-loop-ebea1cb9c1e248fb837f92ae653ecfed\summary.json")
 C195Summary=(Join-Path $Root "runs\c195-v5e-budget12-exhaustion-cc15ada267384d969c1c2f1a686ef7e4\summary.json")
...
try{
 & {
   Write-Output "=== C199 repository preflight ==="
   ...
   .\tools\run_c199.ps1 @runArgs
 } *>&1 | Tee-Object -FilePath $log
}catch{$failure=$_}
finally{
 try{.\tools\publish_experiment_log.ps1 -ExperimentId C199 -LogPath $log -ExecutionHead $ExpectedHead}
 catch{
   if($null -ne $failure){throw "C199 execution failed and log publication also failed. Execution error: $($failure.Exception.Message); publish error: $($_.Exception.Message)"}
   throw
 }
}
