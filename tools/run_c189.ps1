...
$ParentPaths = @($C188Summary, $C187Summary, $C186Summary, $C185Summary, $C184Summary, $C183Summary,
    $C182Summary, $C181Summary, $C180Summary, $C179Summary, $C178Summary, $C177Summary,
    $C176Summary, $C174Summary) | ForEach-Object { (Resolve-Path -LiteralPath $_).Path }
$Names = @("c188","c187","c186","c185","c184","c183","c182","c181","c180","c179",
    "c178","c177","c176","c174")
$Hashes = @(
...
Confirm-Repository
Write-Output "=== FOLD C189 V5-E live multi-missing target acquisition ==="
Write-Output "repository = akakishi04/fold; branch = feat/sft-target-loss; commit = $ExpectedHead"
Write-Output "all nine frozen accepted C188 selectors; three frozen C181 INTERNAL_SEMANTICS bases; no training"
Write-Output "1768 PILOT NEEDS rows x2 selected-fact bits x9 selectors =31824 live episodes /18 blocks"
Write-Output "one shared initial forward emits necessity+target; exact learned target drives C172/C173 RETRIEVE"
Write-Output "identity/original C174 local layout only; max1 acquisition; post necessity once; no second acquisition/answer/proof"
Write-Output "expected_focused_tests = 1449 (1413 existing + 36 new); Gate_E = NOT_PASSED"
$Precheck = @'
from pathlib import Path
import sys
from fold_lm.v05_benchmarks import gate_e_c189_live_multimissing_target as b
b.precheck(Path(sys.argv[1]),*(Path(x) for x in sys.argv[2:15]),Path.cwd())
print('source_and_artifact_precheck = PASS; accepted C188 selector artifacts fixed',flush=True)
'@
& $Python -u -c $Precheck @ParentPaths
if ($LASTEXITCODE -ne 0) { throw "C189 source/artifact precheck failed" }
$Regression = @'
from pathlib import Path
import sys,unittest
from fold_lm.v05_benchmarks import gate_e_c189_live_multimissing_target as b
...
assert suite.countTestCases()==1449,f'Expected1449 tests, got{suite.countTestCases()}'
r=unittest.TextTestRunner(verbosity=2).run(suite)
sys.exit(0 if r.wasSuccessful() else 1)
'@
$Out = Join-Path $Root ("runs\c189-v5e-live-multimissing-target-" + [guid]::NewGuid().ToString("N"))
Write-Output "output_dir = $Out"
$Completed = $false
try {
    Write-Output "=== focused regression ==="
    & $Python -u -c $Regression
    if ($LASTEXITCODE -ne 0) { throw "C189 regression failed; do not run benchmark" }
    Confirm-Repository
    $Cli = @("--output-dir",$Out,"--expected-head",$ExpectedHead)
    for ($i=0; $i -lt $ParentPaths.Count; $i++) { $Cli += @("--$($Names[$i])-summary",$ParentPaths[$i]) }
    & $Python -u -m fold_lm.v05_benchmarks.gate_e_c189_live_multimissing_target @Cli
    if ($LASTEXITCODE -ne 0) { throw "C189 execution failed; preserve invalid.json" }
    $Postcheck = @'
from pathlib import Path
import sys
from fold_lm.v05_benchmarks import gate_e_c189_live_multimissing_target as b
...
out=Path(sys.argv[1])
b.precheck(Path(sys.argv[2]),*(Path(x) for x in sys.argv[3:16]),Path.cwd())
...
print('live_failed_episodes =',p['totals']['failed'])
print('target_errors =',p['totals']['target_error'])
print('post_errors =',p['totals']['post_error'])
print('actual_file_reads =',p['actual_file_reads'])
print('candidate_gate_passed =',b.gate(p['records'],p['selector_replay']))
'@
    & $Python -u -c $Postcheck $Out @ParentPaths $ExpectedHead
    if ($LASTEXITCODE -ne 0) { throw "C189 artifact postcheck failed" }
    $Completed = $true
}
finally {
    Write-Output "=== C189 POSTCHECK ==="
    Confirm-Repository
