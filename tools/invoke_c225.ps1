...
if ($active.Count -ne 1 -or $active[0].Groups["id"].Value -ne "225") { Skip-Invocation "STALE_EXPERIMENT"; return }
$runnerPath = Join-Path $Root "tools\run_c225.ps1"
$runnerTokens = $null
...
$runArgs = @{
    ...
    C224Summary = (Join-Path $Root "runs\c224-v5f-history-cost-audit-7d510ad2c98647bfbabeabe91de81381\summary.json")
}
$failure = $null
try {
    & {
        Write-Output "=== C225 repository preflight ==="
        Write-Output "execution_head = $headNow"
        .\tools\run_c225.ps1 @runArgs
    } *>&1 | Tee-Object -FilePath $log
}
catch { $failure = $_ }
finally {
    try { .\tools\publish_experiment_log.ps1 -ExperimentId C225 -LogPath $log -ExecutionHead $ExpectedHead }
    catch {
        if ($null -ne $failure) { throw "C225 execution and publication failed: $($failure.Exception.Message); $($_.Exception.Message)" }
        throw
    }
}
