...
if ($active.Count -ne 1 -or $active[0].Groups["id"].Value -ne "255") { Skip-Invocation "STALE_EXPERIMENT"; return }
$runnerPath = Join-Path $Root "tools\run_c255.ps1"
$runnerTokens = $null
...
$runArgs = @{
    ...
    C254Summary = (Join-Path $Root "runs\c254-v5b-paired-residual-e324dc81a0364f92820cf8da7f3a0172\summary.json")
    C253Summary = (Join-Path $Root "runs\c253-v5b-frozen-residual-97fc39ef6e8b42cba8c2d1933ddfcbf6\summary.json")
    C252Summary = (Join-Path $Root "runs\c252-v5b-precore-query-ea8199d8ca794acba051ca5f65a391b3\summary.json")
}
$failure = $null
try {
    & {
        Write-Output "=== C255 repository preflight ==="
        Write-Output "execution_head = $headNow"
        .\tools\run_c255.ps1 @runArgs
    } *>&1 | Tee-Object -FilePath $log
}
catch { $failure = $_ }
finally {
    try { .\tools\publish_experiment_log.ps1 -ExperimentId C255 -LogPath $log -ExecutionHead $ExpectedHead }
    catch {
        if ($null -ne $failure) { throw "C255 execution and publication failed: $($failure.Exception.Message); $($_.Exception.Message)" }
        throw
    }
}
