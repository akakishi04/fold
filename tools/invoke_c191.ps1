...
try{
 & {
   Write-Output "=== C191 repository preflight ==="
   ...
   .\tools\run_c191.ps1 @runArgs
 } *>&1 | Tee-Object -FilePath $log
}catch{$failure=$_}
finally{
 try{.\tools\publish_experiment_log.ps1 -ExperimentId C191 -LogPath $log -ExecutionHead $ExpectedHead}
 catch{
   if($null -ne $failure){throw "C191 execution failed and log publication also failed. Execution error: $($failure.Exception.Message); publish error: $($_.Exception.Message)"}
   throw
 }
}
