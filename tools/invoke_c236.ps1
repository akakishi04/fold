...
if ($active.Count -ne 1 -or $active[0].Groups["id"].Value -ne "236") { Skip-Invocation "STALE_EXPERIMENT"; return }
$runnerPath = Join-Path $Root "tools\run_c236.ps1"
$runnerTokens = $null
...
$runArgs = @{
    ...
    C235Summary = (Join-Path $Root "runs\c235-v5b-frozen-binding-diagnostic-17d75b38b35249a297bde78cee0f1d43\summary.json")
    C234Summary = (Join-Path $Root "runs\c234-v5b-context-binding-fb381b067fc54cb3a46dbf3897ece200\summary.json")
}
$failure = $null
try {
    & {
        Write-Output "=== C236 repository preflight ==="
        Write-Output "execution_head = $headNow"
        .\tools\run_c236.ps1 @runArgs
    } *>&1 | Tee-Object -FilePath $log
}
catch { $failure = $_ }
finally {
    try { .\tools\publish_experiment_log.ps1 -ExperimentId C236 -LogPath $log -ExecutionHead $ExpectedHead }
    catch {
        if ($null -ne $failure) { throw "C236 execution and publication failed: $($failure.Exception.Message); $($_.Exception.Message)" }
        throw
    }
}
