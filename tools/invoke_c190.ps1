...
try {
    & {
        Write-Output "=== C190 repository preflight ==="
    ...
    try {
        .\tools\publish_experiment_log.ps1 -ExperimentId C190 -LogPath $log -ExecutionHead $ExpectedHead
    }
    catch {
        if ($null -ne $failure) {
            throw "C190 execution failed and log publication also failed. Execution error: $($failure.Exception.Message); publish error: $($_.Exception.Message)"
        }
        throw
    }
}
