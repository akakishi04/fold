...
if ($active.Count -ne 1 -or $active[0].Groups["id"].Value -ne "260") { Skip-Invocation "STALE_EXPERIMENT"; return }
$runnerPath = Join-Path $Root "tools\run_c260.ps1"
$runnerTokens = $null
...
$runArgs = @{
    ...
    C259Summary = (Join-Path $Root "runs\c259-v5b-order-coverage-7042b90d1ed54cc0858291e661d329e8\summary.json")
}
$failure = $null
try {
    & {
        Write-Output "=== C260 repository preflight ==="
        Write-Output "execution_head = $headNow"
        .\tools\run_c260.ps1 @runArgs
    } *>&1 | Tee-Object -FilePath $log
}
catch { $failure = $_ }
finally {
    try { .\tools\publish_experiment_log.ps1 -ExperimentId C260 -LogPath $log -ExecutionHead $ExpectedHead }
    catch {
        if ($null -ne $failure) { throw "C260 execution and publication failed: $($failure.Exception.Message); $($_.Exception.Message)" }
        throw
    }
}
