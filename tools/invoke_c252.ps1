...
if ($active.Count -ne 1 -or $active[0].Groups["id"].Value -ne "252") { Skip-Invocation "STALE_EXPERIMENT"; return }

$runnerPath = Join-Path $Root "tools\run_c252.ps1"
$runnerTokens = $null
...
$runArgs = @{
    ...
    C251Summary = (Join-Path $Root "runs\c251-v5b-precore-read-dcc6ca85fe504ea7a71db741ab1558b1\summary.json")
}

$failure = $null
try {
    & {
        Write-Output "=== C252 repository preflight ==="
        Write-Output "execution_head = $headNow"
        .\tools\run_c252.ps1 @runArgs
    } *>&1 | Tee-Object -FilePath $log
}
catch { $failure = $_ }
finally {
    try { .\tools\publish_experiment_log.ps1 -ExperimentId C252 -LogPath $log -ExecutionHead $ExpectedHead }
    catch {
        if ($null -ne $failure) { throw "C252 execution and publication failed: $($failure.Exception.Message); $($_.Exception.Message)" }
        throw
    }
}
