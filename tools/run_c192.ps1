...
$ParentPaths=@($C191Summary,$C190Summary,$C189Summary,$C188Summary,$C187Summary,$C186Summary,
    $C185Summary,$C184Summary,$C183Summary,$C182Summary,$C181Summary,$C180Summary,$C179Summary,
    $C178Summary,$C177Summary,$C176Summary,$C174Summary) | ForEach-Object {(Resolve-Path -LiteralPath $_).Path}
$Names=@("c191","c190","c189","c188","c187","c186","c185","c184","c183","c182","c181","c180",
...
Confirm-Repository
Write-Output "=== FOLD C192 V5-E post3 shadow necessity ==="
Write-Output "repository = akakishi04/fold; branch = feat/sft-target-loss; commit = $ExpectedHead"
Write-Output "same accepted C191 live path; shadow probe only after third acquisition; no scheduler debit/state mutation"
Write-Output "expected_focused_tests = 1533 (1509 existing + 24 new); Gate_E = NOT_PASSED"

$Precheck=@'
from pathlib import Path
import sys
from fold_lm.v05_benchmarks import gate_e_c192_post3_shadow_necessity as b
b.precheck(Path(sys.argv[1]),*(Path(x) for x in sys.argv[2:18]),Path.cwd())
print('source_and_artifact_precheck = PASS; accepted C191 result/artifacts fixed',flush=True)
'@
& $Python -u -c $Precheck @ParentPaths
if($LASTEXITCODE -ne 0){throw "C192 source/artifact precheck failed"}
...
assert suite.countTestCases()==1533,f'Expected1533 tests, got{suite.countTestCases()}'
r=unittest.TextTestRunner(verbosity=2).run(suite)
sys.exit(0 if r.wasSuccessful() else 1)
'@

$Out=Join-Path $Root ("runs\c192-v5e-post3-shadow-" + [guid]::NewGuid().ToString("N"))
Write-Output "output_dir = $Out"
$Completed=$false
try{
    Write-Output "=== focused regression ==="
    & $Python -u -c $Regression
    if($LASTEXITCODE -ne 0){throw "C192 regression failed; do not run benchmark"}
    Confirm-Repository
    $Cli=@("--output-dir",$Out,"--expected-head",$ExpectedHead)
    for($i=0;$i -lt $ParentPaths.Count;$i++){$Cli+=@("--$($Names[$i])-summary",$ParentPaths[$i])}
    & $Python -u -m fold_lm.v05_benchmarks.gate_e_c192_post3_shadow_necessity @Cli
    if($LASTEXITCODE -ne 0){throw "C192 execution failed; preserve invalid.json"}
...
out=Path(sys.argv[1])
b.precheck(Path(sys.argv[2]),*(Path(x) for x in sys.argv[3:19]),Path.cwd())
...
print('shadow_rows =',p['shadow_rows'])
print('shadow_errors =',p['totals']['shadow_error'])
print('runtime_mutations =',p['totals']['runtime_mutation'])
print('candidate_gate_passed =',b.gate(p['records']))
'@
    & $Python -u -c $Postcheck $Out @ParentPaths $ExpectedHead
    if($LASTEXITCODE -ne 0){throw "C192 artifact postcheck failed"}
    $Completed=$true
}
finally{
    Write-Output "=== C192 POSTCHECK ==="
    Confirm-Repository
