...
if ($active.Count -ne 1 -or $active[0].Groups["id"].Value -ne "258") { Skip-Invocation "STALE_EXPERIMENT"; return }
$runnerPath = Join-Path $Root "tools\run_c258.ps1"
$runnerTokens = $null
...
$runArgs = @{
    ...
    C257Summary = (Join-Path $Root "runs\c257-v5b-unseen-order-705d486739b44a56bbed34fae66ace89\summary.json")
    C256Summary = (Join-Path $Root "runs\c256-v5b-three-entity-3f95a0d190e4424f91634cb8813d7987\summary.json")
}
$failure = $null
try {
    & {
        Write-Output "=== C258 repository preflight ==="
        Write-Output "execution_head = $headNow"
        .\tools\run_c258.ps1 @runArgs
    } *>&1 | Tee-Object -FilePath $log
}
catch { $failure = $_ }
finally {
    try { .\tools\publish_experiment_log.ps1 -ExperimentId C258 -LogPath $log -ExecutionHead $ExpectedHead }
    catch {
        if ($null -ne $failure) { throw "C258 execution and publication failed: $($failure.Exception.Message); $($_.Exception.Message)" }
        throw
    }
}
