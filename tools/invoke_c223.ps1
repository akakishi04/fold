...
if ($active.Count -ne 1 -or $active[0].Groups["id"].Value -ne "223") { Skip-Invocation "STALE_EXPERIMENT"; return }
$runnerPath = Join-Path $Root "tools\run_c223.ps1"
$runnerTokens = $null
...
$runArgs = @{
    ...
    C222Summary = (Join-Path $Root "runs\c222-v5f-withdrawal-lifecycle-60ba479f7d2b46ff957b5d2a21bf865c\summary.json")
}
$failure = $null
try {
    & {
        Write-Output "=== C223 repository preflight ==="
        Write-Output "execution_head = $headNow"
        .\tools\run_c223.ps1 @runArgs
    } *>&1 | Tee-Object -FilePath $log
}
catch { $failure = $_ }
finally {
    try { .\tools\publish_experiment_log.ps1 -ExperimentId C223 -LogPath $log -ExecutionHead $ExpectedHead }
    catch {
        if ($null -ne $failure) { throw "C223 execution and publication failed: $($failure.Exception.Message); $($_.Exception.Message)" }
        throw
    }
}
