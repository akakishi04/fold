...
if ($active.Count -ne 1 -or $active[0].Groups["id"].Value -ne "246") { Skip-Invocation "STALE_EXPERIMENT"; return }
$runnerPath = Join-Path $Root "tools\run_c246.ps1"
$runnerTokens = $null
...
$runArgs = @{
    ...
    C245Summary = (Join-Path $Root "runs\c245-v5b-selective-evidence-484b9afa921e4fe4a3430a1d7a88e4d4\summary.json")
    C244Summary = (Join-Path $Root "runs\c244-v5b-two-partner-recombination-63a64987412a494291853ed0afb9c639\summary.json")
}
$failure = $null
try {
    & {
        Write-Output "=== C246 repository preflight ==="
        Write-Output "execution_head = $headNow"
        .\tools\run_c246.ps1 @runArgs
    } *>&1 | Tee-Object -FilePath $log
}
catch { $failure = $_ }
finally {
    try { .\tools\publish_experiment_log.ps1 -ExperimentId C246 -LogPath $log -ExecutionHead $ExpectedHead }
    catch {
        if ($null -ne $failure) { throw "C246 execution and publication failed: $($failure.Exception.Message); $($_.Exception.Message)" }
        throw
    }
}
