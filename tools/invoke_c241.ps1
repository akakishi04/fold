...
if ($active.Count -ne 1 -or $active[0].Groups["id"].Value -ne "241") { Skip-Invocation "STALE_EXPERIMENT"; return }

$runnerPath = Join-Path $Root "tools\run_c241.ps1"
$runnerTokens = $null
...
$runArgs = @{
    ...
    C240Summary = (Join-Path $Root "runs\c240-v5b-saved-position-audit-4b604be67f5b47f0b18cabdb2513d12d\summary.json")
...
try {
    & {
        Write-Output "=== C241 repository preflight ==="
        Write-Output "execution_head = $headNow"
        .\tools\run_c241.ps1 @runArgs
    } *>&1 | Tee-Object -FilePath $log
}
catch { $failure = $_ }
finally {
    try { .\tools\publish_experiment_log.ps1 -ExperimentId C241 -LogPath $log -ExecutionHead $ExpectedHead }
    catch {
        if ($null -ne $failure) { throw "C241 execution and publication failed: $($failure.Exception.Message); $($_.Exception.Message)" }
        throw
    }
}
