...
if (-not $activeMatch.Success -or $activeMatch.Groups["id"].Value -ne "216") {
    $active = if ($activeMatch.Success) { "C" + $activeMatch.Groups["id"].Value } else { "UNRESOLVED" }
    Skip-Invocation -Reason "STALE_EXPERIMENT" -Detail "requested=C216 active=$active"
    return
}

$runnerPath = Join-Path $Root "tools\run_c216.ps1"
$runnerTokens = $null
...
$runArgs = @{
    ...
    C215Summary = (Join-Path $Root "runs\c215-v5f-h1-h2-chunk-commit-06e5fce5f05f46d0be1f86b8ec119877\summary.json")
}

$failure = $null
try {
    & {
        Write-Output "=== C216 repository preflight ==="
        Write-Output "branch = $branchNow"
        Write-Output "execution_head = $headNow"
        .\tools\run_c216.ps1 @runArgs
    ...
    try {
        .\tools\publish_experiment_log.ps1 -ExperimentId C216 -LogPath $log -ExecutionHead $ExpectedHead
    }
    catch {
        if ($null -ne $failure) {
            throw "C216 execution failed and log publication also failed. Execution error: $($failure.Exception.Message); publish error: $($_.Exception.Message)"
        }
        throw
    }
}
