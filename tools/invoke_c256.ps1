...
if($active.Count -ne 1 -or $active[0].Groups["id"].Value -ne "256"){Skip-Invocation "STALE_EXPERIMENT";return}
$runnerPath=Join-Path $Root "tools\run_c256.ps1"
...
$runArgs=@{
    ...
    C255Summary=(Join-Path $Root "runs\c255-v5b-value-residual-8203653a07964d9bb549c272a8b9b943\summary.json")
}
$failure=$null
try {
    & {
        Write-Output "=== C256 repository preflight ==="
        Write-Output "execution_head = $headNow"
        .\tools\run_c256.ps1 @runArgs
    } *>&1 | Tee-Object -FilePath $log
}
catch {$failure=$_}
finally {
    try {.\tools\publish_experiment_log.ps1 -ExperimentId C256 -LogPath $log -ExecutionHead $ExpectedHead}
    catch {
        if($null -ne $failure){throw "C256 execution and publication failed: $($failure.Exception.Message); $($_.Exception.Message)"}
        throw
    }
}
