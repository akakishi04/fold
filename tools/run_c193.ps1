...
$ParentPaths=@($C192Summary,$C191Summary,$C190Summary,$C189Summary,$C188Summary,$C187Summary,
    $C186Summary,$C185Summary,$C184Summary,$C183Summary,$C182Summary,$C181Summary,$C180Summary,
    $C179Summary,$C178Summary,$C177Summary,$C176Summary,$C174Summary) | ForEach-Object {(Resolve-Path -LiteralPath $_).Path}
$Names=@("c192","c191","c190","c189","c188","c187","c186","c185","c184","c183","c182","c181",
...
Confirm-Repository
Write-Output "=== FOLD C193 V5-E budget13 authoritative closure ==="
Write-Output "repository = akakishi04/fold; branch = feat/sft-target-loss; commit = $ExpectedHead"
Write-Output "changed variable = initial internal_remaining 12 -> 13 only"
Write-Output "same85824 coherent-world episodes; frozen C181/C188; full path rescored because budget is model-visible"
Write-Output "expected_focused_tests = 1557 (1533 existing +24 new); Gate_E = NOT_PASSED"

$Precheck=@'
from pathlib import Path
import sys
from fold_lm.v05_benchmarks import gate_e_c193_budget13_authoritative_closure as b
b.precheck(Path(sys.argv[1]),*(Path(x) for x in sys.argv[2:19]),Path.cwd())
print('source_and_artifact_precheck = PASS; accepted C192 lineage fixed',flush=True)
'@
& $Python -u -c $Precheck @ParentPaths
if($LASTEXITCODE -ne 0){throw "C193 source/artifact precheck failed"}
...
assert suite.countTestCases()==1557,f'Expected1557 tests, got{suite.countTestCases()}'
r=unittest.TextTestRunner(verbosity=2).run(suite)
sys.exit(0 if r.wasSuccessful() else 1)
'@

$Out=Join-Path $Root ("runs\c193-v5e-budget13-closure-" + [guid]::NewGuid().ToString("N"))
Write-Output "output_dir = $Out"
$Completed=$false
try{
    Write-Output "=== focused regression ==="
    & $Python -u -c $Regression
    if($LASTEXITCODE -ne 0){throw "C193 regression failed; do not run benchmark"}
    Confirm-Repository
    $Cli=@("--output-dir",$Out,"--expected-head",$ExpectedHead)
    for($i=0;$i -lt $ParentPaths.Count;$i++){$Cli+=@("--$($Names[$i])-summary",$ParentPaths[$i])}
    & $Python -u -m fold_lm.v05_benchmarks.gate_e_c193_budget13_authoritative_closure @Cli
    if($LASTEXITCODE -ne 0){throw "C193 execution failed; preserve invalid.json"}
...
out=Path(sys.argv[1])
b.precheck(Path(sys.argv[2]),*(Path(x) for x in sys.argv[3:20]),Path.cwd())
...
print('final_decision_errors =',p['totals']['final_decision_error'])
print('candidate_gate_passed =',b.gate(p['records']))
'@
    & $Python -u -c $Postcheck $Out @ParentPaths $ExpectedHead
    if($LASTEXITCODE -ne 0){throw "C193 artifact postcheck failed"}
    $Completed=$true
}
finally{
    Write-Output "=== C193 POSTCHECK ==="
    Confirm-Repository
