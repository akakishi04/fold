...
$ParentPaths=@($C193Summary,$C192Summary,$C191Summary,$C190Summary,$C189Summary,$C188Summary,
    $C187Summary,$C186Summary,$C185Summary,$C184Summary,$C183Summary,$C182Summary,$C181Summary,
    $C180Summary,$C179Summary,$C178Summary,$C177Summary,$C176Summary,$C174Summary) |
    ForEach-Object {(Resolve-Path -LiteralPath $_).Path}
$Names=@("c193","c192","c191","c190","c189","c188","c187","c186","c185","c184","c183",
...
Confirm-Repository
Write-Output "=== FOLD C194 V5-E generic bounded loop equivalence ==="
Write-Output "repository = akakishi04/fold; branch = feat/sft-target-loss; commit = $ExpectedHead"
Write-Output "changed variable = hand-unrolled phases -> one state-driven bounded loop"
Write-Output "same budget13 / frozen C181+C188 / 85824 coherent-world episodes / C172+C173"
Write-Output "exact accepted C193 prediction/logit/read-depth replay required"
Write-Output "expected_focused_tests = 1581 (1557 existing +24 new); Gate_E = NOT_PASSED"

$Precheck=@'
from pathlib import Path
import sys
from fold_lm.v05_benchmarks import gate_e_c194_generic_bounded_loop as b
b.precheck(Path(sys.argv[1]),*(Path(x) for x in sys.argv[2:20]),Path.cwd())
print('source_and_artifact_precheck = PASS; accepted C193 result/artifacts fixed',flush=True)
'@
& $Python -u -c $Precheck @ParentPaths
if($LASTEXITCODE -ne 0){throw "C194 source/artifact precheck failed"}
...
assert suite.countTestCases()==1581,f'Expected1581 tests, got{suite.countTestCases()}'
r=unittest.TextTestRunner(verbosity=2).run(suite)
sys.exit(0 if r.wasSuccessful() else 1)
'@

$Out=Join-Path $Root ("runs\c194-v5e-generic-loop-" + [guid]::NewGuid().ToString("N"))
Write-Output "output_dir = $Out"
$Completed=$false
try{
    Write-Output "=== focused regression ==="
    & $Python -u -c $Regression
    if($LASTEXITCODE -ne 0){throw "C194 regression failed; do not run benchmark"}
    Confirm-Repository
    $Cli=@("--output-dir",$Out,"--expected-head",$ExpectedHead)
    for($i=0;$i -lt $ParentPaths.Count;$i++){$Cli+=@("--$($Names[$i])-summary",$ParentPaths[$i])}
    & $Python -u -m fold_lm.v05_benchmarks.gate_e_c194_generic_bounded_loop @Cli
    if($LASTEXITCODE -ne 0){throw "C194 execution failed; preserve invalid.json"}
...
out=Path(sys.argv[1])
b.precheck(Path(sys.argv[2]),*(Path(x) for x in sys.argv[3:21]),Path.cwd())
p=audit.read_json(out/'summary.json');b.validate_result(p)
assert p['commit_sha']==sys.argv[21]
...
print('parent_replay_errors =',p['totals']['parent_replay_error'])
print('parent_block_mismatches =',p['totals']['parent_block_mismatch'])
print('third_reads =',p['totals']['third_reads'])
print('final_decision_rows =',p['totals']['final_decision_rows'])
print('candidate_gate_passed =',b.gate(p['records']))
'@
    & $Python -u -c $Postcheck $Out @ParentPaths $ExpectedHead
    if($LASTEXITCODE -ne 0){throw "C194 artifact postcheck failed"}
    $Completed=$true
}
finally{
    Write-Output "=== C194 POSTCHECK ==="
    Confirm-Repository
