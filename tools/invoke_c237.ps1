...
if ($active.Count -ne 1 -or $active[0].Groups["id"].Value -ne "237") { Skip-Invocation "STALE_EXPERIMENT"; return }
$runnerPath = Join-Path $Root "tools\run_c237.ps1"
$runnerTokens = $null
...
try {
    & {
        Write-Output "=== C237 repository preflight ==="
        Write-Output "execution_head = $headNow"
        .\tools\run_c237.ps1 @runArgs
    } *>&1 | Tee-Object -FilePath $log
}
catch { $failure = $_ }
finally {
    try { .\tools\publish_experiment_log.ps1 -ExperimentId C237 -LogPath $log -ExecutionHead $ExpectedHead }
    catch {
        if ($null -ne $failure) { throw "C237 execution and publication failed: $($failure.Exception.Message); $($_.Exception.Message)" }
        throw
    }
}
