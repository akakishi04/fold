...
if ($active.Count -ne 1 -or $active[0].Groups["id"].Value -ne "239") { Skip-Invocation "STALE_EXPERIMENT"; return }
$runnerPath = Join-Path $Root "tools\run_c239.ps1"
$runnerTokens = $null
...
$runArgs = @{
    ...
    C238Summary = (Join-Path $Root "runs\c238-v5b-complete-cohort-sampler-df6538e306784434bdcbb2848e4c278d\summary.json")
}
$failure = $null
try {
    & {
        Write-Output "=== C239 repository preflight ==="
        Write-Output "execution_head = $headNow"
        .\tools\run_c239.ps1 @runArgs
    } *>&1 | Tee-Object -FilePath $log
}
catch { $failure = $_ }
finally {
    try { .\tools\publish_experiment_log.ps1 -ExperimentId C239 -LogPath $log -ExecutionHead $ExpectedHead }
    catch {
        if ($null -ne $failure) { throw "C239 execution and publication failed: $($failure.Exception.Message); $($_.Exception.Message)" }
        throw
    }
}
