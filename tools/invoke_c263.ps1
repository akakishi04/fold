...
if ($active.Count -ne 1 -or $active[0].Groups["id"].Value -ne "263") { Skip-Invocation "STALE_EXPERIMENT"; return }
$runnerPath = Join-Path $Root "tools\run_c263.ps1"
$runnerTokens = $null
...
$runArgs = @{
    ...
    C262Summary = (Join-Path $Root "runs\c262-v5b-batch-order-b955446fc91846aa9fea770cd36b9276\summary.json")
}
$failure = $null
try {
    & {
        Write-Output "=== C263 repository preflight ==="
        Write-Output "execution_head = $headNow"
        .\tools\run_c263.ps1 @runArgs
    } *>&1 | Tee-Object -FilePath $log
}
catch { $failure = $_ }
finally {
    try { .\tools\publish_experiment_log.ps1 -ExperimentId C263 -LogPath $log -ExecutionHead $ExpectedHead }
    catch {
        if ($null -ne $failure) { throw "C263 execution and publication failed: $($failure.Exception.Message); $($_.Exception.Message)" }
        throw
    }
}
