...
if ($active.Count -ne 1 -or $active[0].Groups["id"].Value -ne "249") { Skip-Invocation "STALE_EXPERIMENT"; return }
$runnerPath = Join-Path $Root "tools\run_c249.ps1"
$runnerTokens = $null
...
try {
    & {
        Write-Output "=== C249 repository preflight ==="
        Write-Output "execution_head = $headNow"
        .\tools\run_c249.ps1 @runArgs
    } *>&1 | Tee-Object -FilePath $log
}
catch { $failure = $_ }
finally {
    try { .\tools\publish_experiment_log.ps1 -ExperimentId C249 -LogPath $log -ExecutionHead $ExpectedHead }
    catch {
        if ($null -ne $failure) { throw "C249 execution and publication failed: $($failure.Exception.Message); $($_.Exception.Message)" }
        throw
    }
}
