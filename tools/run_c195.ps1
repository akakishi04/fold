...
$ParentPaths=@($C194Summary,$C193Summary,$C192Summary,$C191Summary,$C190Summary,$C189Summary,
    $C188Summary,$C187Summary,$C186Summary,$C185Summary,$C184Summary,$C183Summary,$C182Summary,
    $C181Summary,$C180Summary,$C179Summary,$C178Summary,$C177Summary,$C176Summary,$C174Summary) |
    ForEach-Object {(Resolve-Path -LiteralPath $_).Path}
$Names=@("c194","c193","c192","c191","c190","c189","c188","c187","c186","c185","c184",
...
Confirm-Repository
Write-Output "=== FOLD C195 V5-E generic-loop budget12 exhaustion safety ==="
Write-Output "repository = akakishi04/fold; branch = feat/sft-target-loss; commit = $ExpectedHead"
Write-Output "changed variable = initial internal_remaining13 ->12 only; C194 generic loop reused unchanged"
Write-Output "accepted C191 budget12 predictions/read depths/exhaustion are the exact reference"
Write-Output "expected_focused_tests = 1605 (1581 existing +24 new); Gate_E = NOT_PASSED"

$Precheck=@'
from pathlib import Path
import sys
from fold_lm.v05_benchmarks import gate_e_c195_generic_loop_budget12_exhaustion as b
b.precheck(Path(sys.argv[1]),*(Path(x) for x in sys.argv[2:21]),Path.cwd())
print('source_and_artifact_precheck = PASS; accepted C194 parent and C191 reference fixed',flush=True)
'@
& $Python -u -c $Precheck @ParentPaths
if($LASTEXITCODE -ne 0){throw "C195 source/artifact precheck failed"}
...
assert suite.countTestCases()==1605,f'Expected1605 tests, got{suite.countTestCases()}'
r=unittest.TextTestRunner(verbosity=2).run(suite)
sys.exit(0 if r.wasSuccessful() else 1)
'@

$Out=Join-Path $Root ("runs\c195-v5e-budget12-exhaustion-" + [guid]::NewGuid().ToString("N"))
Write-Output "output_dir = $Out"
$Completed=$false
try{
    Write-Output "=== focused regression ==="
    & $Python -u -c $Regression
    if($LASTEXITCODE -ne 0){throw "C195 regression failed; do not run benchmark"}
    Confirm-Repository
    $Cli=@("--output-dir",$Out,"--expected-head",$ExpectedHead)
    for($i=0;$i -lt $ParentPaths.Count;$i++){$Cli+=@("--$($Names[$i])-summary",$ParentPaths[$i])}
    & $Python -u -m fold_lm.v05_benchmarks.gate_e_c195_generic_loop_budget12_exhaustion @Cli
    if($LASTEXITCODE -ne 0){throw "C195 execution failed; preserve invalid.json"}
...
out=Path(sys.argv[1])
b.precheck(Path(sys.argv[2]),*(Path(x) for x in sys.argv[3:22]),Path.cwd())
p=audit.read_json(out/'summary.json');b.validate_result(p)
assert p['commit_sha']==sys.argv[22]
...
print('reference_replay_errors =',p['totals']['reference_replay_error'])
print('reference_block_mismatches =',p['totals']['reference_block_mismatch'])
print('third_reads =',p['totals']['third_reads'])
print('exhausted_rows =',p['totals']['exhausted_rows'])
print('fake_final_decisions =',p['totals']['fake_final_decision'])
print('candidate_gate_passed =',b.gate(p['records']))
'@
    & $Python -u -c $Postcheck $Out @ParentPaths $ExpectedHead
    if($LASTEXITCODE -ne 0){throw "C195 artifact postcheck failed"}
    $Completed=$true
}
finally{
    Write-Output "=== C195 POSTCHECK ==="
    Confirm-Repository
