...
if ($active.Count -ne 1 -or $active[0].Groups["id"].Value -ne "232") { Skip-Invocation "STALE_EXPERIMENT"; return }
$runnerPath = Join-Path $Root "tools\run_c232.ps1"
$runnerTokens = $null
...
$runArgs = @{
    ...
    C231Summary = (Join-Path $Root "runs\c231-v5b-byte-eval-9e4c4cd1513a4b3c97e598252f6300c5\summary.json")
}
$failure = $null
try {
    & {
        Write-Output "=== C232 repository preflight ==="
        Write-Output "execution_head = $headNow"
        .\tools\run_c232.ps1 @runArgs
    } *>&1 | Tee-Object -FilePath $log
}
catch { $failure = $_ }
finally {
    try { .\tools\publish_experiment_log.ps1 -ExperimentId C232 -LogPath $log -ExecutionHead $ExpectedHead }
    catch {
        if ($null -ne $failure) { throw "C232 execution and publication failed: $($failure.Exception.Message); $($_.Exception.Message)" }
        throw
    }
}
