...
$ParentPaths = @($C186Summary, $C185Summary, $C184Summary, $C183Summary, $C182Summary, $C181Summary,
    $C180Summary, $C179Summary, $C178Summary, $C177Summary, $C176Summary, $C174Summary) |
    ForEach-Object { (Resolve-Path -LiteralPath $_).Path }
$Names = @("c186", "c185", "c184", "c183", "c182", "c181", "c180", "c179", "c178", "c177", "c176", "c174")
$Hashes = @(
...
Confirm-Repository
Write-Output "=== FOLD C187 V5-E restricted acquisition necessity ==="
Write-Output "repository = akakishi04/fold; branch = feat/sft-target-loss; commit = $ExpectedHead"
Write-Output "six frozen C181 checkpoints; source seeds181001/2/3; MISSING_RULE and NEVER_QUERY; fresh_seeds=0"
Write-Output "3712 rows x2 layouts x5 conditions x8 policies =296960 episodes /80 blocks"
Write-Output "allowed0/1, permission denied, provider unavailable, acquisition budget zero; unchanged C185 driver"
Write-Output "max1 proposal and2 decisions; denied post9internal/step10; no observation, retry or answer fallback"
Write-Output "expected_focused_tests = 1377 (1341 existing + 36 new); Gate_E = NOT_PASSED"
$Precheck = @'
from pathlib import Path
import sys
from fold_lm.v05_benchmarks import gate_e_c187_restricted_acquisition as b
b.precheck(*(Path(x) for x in sys.argv[1:13]),Path.cwd())
print('source_and_artifact_precheck = PASS; accepted C185 NPZ schema checked',flush=True)
'@
& $Python -u -c $Precheck @ParentPaths
if ($LASTEXITCODE -ne 0) { throw "C187 source/artifact precheck failed" }
$Regression = @'
from pathlib import Path
import sys,unittest
from fold_lm.v05_benchmarks import gate_e_c187_restricted_acquisition as b
...
assert suite.countTestCases()==1377,f'Expected1377 tests, got{suite.countTestCases()}'
r=unittest.TextTestRunner(verbosity=2).run(suite)
sys.exit(0 if r.wasSuccessful() else 1)
'@
$Out = Join-Path $Root ("runs\c187-v5e-restricted-acquisition-" + [guid]::NewGuid().ToString("N"))
Write-Output "output_dir = $Out"
$Completed = $false
try {
    Write-Output "=== focused regression ==="
    & $Python -u -c $Regression
    if ($LASTEXITCODE -ne 0) { throw "C187 regression failed; do not run benchmark" }
    Confirm-Repository
    $Cli = @("--output-dir", $Out, "--expected-head", $ExpectedHead)
    for ($i = 0; $i -lt $ParentPaths.Count; $i++) { $Cli += @("--$($Names[$i])-summary", $ParentPaths[$i]) }
    & $Python -u -m fold_lm.v05_benchmarks.gate_e_c187_restricted_acquisition @Cli
    if ($LASTEXITCODE -ne 0) { throw "C187 execution failed; preserve invalid.json" }
    $Postcheck = @'
from pathlib import Path
import sys
from fold_lm.v05_benchmarks import gate_e_c187_restricted_acquisition as b
from fold_lm.v05_benchmarks import gate_e_c175_frozen_prediction_audit as audit
out=Path(sys.argv[1]);b.precheck(*(Path(x) for x in sys.argv[2:14]),Path.cwd())
...
print('restricted_provider_calls =',sum(r['provider_calls'] for r in p['records'] if not b.scenario_spec(r['scenario'])['allowed']))
...
'@
    ...
    if ($LASTEXITCODE -ne 0) { throw "C187 artifact postcheck failed" }
    $Completed = $true
}
finally {
    Write-Output "=== C187 POSTCHECK ==="
    Confirm-Repository
