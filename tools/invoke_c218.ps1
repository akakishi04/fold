...
if (-not $activeMatch.Success -or $activeMatch.Groups["id"].Value -ne "218") {
    $active = if ($activeMatch.Success) { "C" + $activeMatch.Groups["id"].Value } else { "UNRESOLVED" }
    Skip-Invocation -Reason "STALE_EXPERIMENT" -Detail "requested=C218 active=$active"
    return
}

$runnerPath = Join-Path $Root "tools\run_c218.ps1"
$runnerTokens = $null
...
$runArgs = @{
    ...
    C217Summary = (Join-Path $Root "runs\c217-v5f-learned-port-selector-6dfc0082313d48aa826750203c887bc6\summary.json")
}

$failure = $null
try {
    & {
        Write-Output "=== C218 repository preflight ==="
        Write-Output "branch = $branchNow"
        Write-Output "execution_head = $headNow"
        .\tools\run_c218.ps1 @runArgs
    ...
    try {
        .\tools\publish_experiment_log.ps1 -ExperimentId C218 -LogPath $log -ExecutionHead $ExpectedHead
    }
    catch {
        if ($null -ne $failure) {
            throw "C218 execution failed and log publication also failed. Execution error: $($failure.Exception.Message); publish error: $($_.Exception.Message)"
        }
        throw
    }
}
