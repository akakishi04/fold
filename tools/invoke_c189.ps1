...
try {
    & {
        Write-Output "=== C189 repository preflight ==="
        $branch = git branch --show-current
        if ($LASTEXITCODE -ne 0 -or $branch -ne "feat/sft-target-loss") {
            throw "Unexpected branch: $branch"
        }
        $remote = git remote get-url origin
        if ($LASTEXITCODE -ne 0 -or $remote -notmatch '[:/]akakishi04/fold(?:\.git)?$') {
        ...
        if ($LASTEXITCODE -ne 0 -or $head -ne $ExpectedHead) {
            throw "Unexpected synchronized HEAD: $head"
        }
        .\tools\run_c189.ps1 @runArgs
    ...
    try {
        .\tools\publish_experiment_log.ps1 -ExperimentId C189 -LogPath $log -ExecutionHead $ExpectedHead
    }
    catch {
        if ($null -ne $failure) {
            throw "C189 execution failed and log publication also failed. Execution error: $($failure.Exception.Message); publish error: $($_.Exception.Message)"
        }
        throw
    }
}
