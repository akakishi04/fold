param(
    [Parameter(Mandatory = $true)][string]$C165Summary,
...
try {
    Write-Output "=== FOLD C166 V5-E live query acquisition budget exhaustion ==="
    ...
    $required = @{}
    $required[$C165Summary] = "7CDFBF97FEC85BB28E3FBC3FE4CBB470EC4497C31DBEC33879D7B442097724C6"
    ...
    Write-Output "stage = V5-E-LIVE-QUERY-BUDGET-EXHAUSTED"
    ...
    Write-Output "decisions = 331776; acquisitions = 82944; restorations = 82944; reads = 165888; vectors = 10616832"
    Write-Output "ANSWERED = 82944 expected; UNRESOLVED/BUDGET_EXHAUSTED = 82944 expected"
    Write-Output "permission = True both; initial_internal_budget = 3 both; acquisition_allowance = 1 versus 0"
    Write-Output "exhausted_adapter_calls = 0; acquisition_debit = 0; internal_decisions = 2 per branch"
    Write-Output "native_control_calls = 165888; adapted_output_calls = 165888; guard_calls = 768"
    Write-Output "training = 0; fresh_seeds = 0; expected_focused_tests = 695"
    Write-Output "changed_variable = initial BudgetState.acquisitions_remaining ONLY"
    ...
    if ($count -ne "695") { throw "Expected 695 focused tests, got $count" }
    & $python -m unittest @tests -v
    Assert-NativeExit "C166 focused regression"
    Write-Output "=== C166 acquisition-budget-only live comparison ==="
    $out = Join-Path "runs" ("c166-v5e-live-budget-exhausted-" + [guid]::NewGuid().ToString("N"))
    & $python -u -m fold_lm.v05_benchmarks.gate_e_c166_live_budget_exhausted `
        --c165-summary $C165Summary --output-dir $out
    Assert-NativeExit "C166 integration execution"
    $summary = Join-Path $out "summary.json"
    $report = Get-Content -LiteralPath $summary -Raw -Encoding UTF8 | ConvertFrom-Json
    if ($report.experiment_id -ne "C166-v5e-live-query-budget-exhausted" -or $report.stage -ne "V5-E-LIVE-QUERY-BUDGET-EXHAUSTED" -or $report.status -notin @("PASS", "FAIL")) { throw "Wrong experiment/result" }
    ...
    $s = $report.summary
    if ($s.training_steps -ne 0 -or $s.fresh_seed_count -ne 0 -or $s.live_query_selection -ne $true -or $s.live_cycle_exercised -ne $true -or $s.structured_result_exercised -ne $true -or $s.acquisition_budget_gate_exercised -ne $true -or $s.answer_generation_exercised -ne $false -or $s.production_state_commit -ne $false -or $s.new_evidence_epoch -ne $false) { throw "Unexpected scope" }
    if (($report.status -eq "PASS") -ne ($s.budget_exhausted_gate_passed -eq $true)) { throw "Status/gate mismatch" }
...
if (-not $postcheckOk) { throw "C166 postcheck failed; do not accept result" }
