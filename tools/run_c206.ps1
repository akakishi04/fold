...
Write-Output "=== C206 authoring syntax preflight ==="
& $Python -m py_compile (Join-Path $Root "fold_lm\v05_benchmarks\gate_e_c206_terminal_derived_result_integration.py") (Join-Path $Root "tests_lm\test_v05_c206_terminal_derived_result_integration.py")
if($LASTEXITCODE -ne 0){throw "C206 Python syntax preflight failed"}
...
import sys
from fold_lm.v05_benchmarks import gate_e_c206_terminal_derived_result_integration as b
b.precheck(
...
print('source_and_artifact_precheck = PASS; accepted C205/C199/C171 lineage fixed',flush=True)
'@
& $Python -u -c $Precheck $C205Summary $C204Summary $C203Summary $C202Summary $C201Summary $C200Summary $C199Summary $C174Summary $C181Summary $C188Summary
if($LASTEXITCODE -ne 0){throw "C206 source/artifact precheck failed"}
...
$Out=Join-Path $Root ("runs\c206-v5e-terminal-derived-" + [guid]::NewGuid().ToString("N"))
Write-Output "output_dir = $Out"
$Completed=$false
try{
    Write-Output "=== focused regression ==="
    & $Python -u -c $Regression
    if($LASTEXITCODE -ne 0){throw "C206 regression failed; do not run diagnostic"}
    Confirm-Repository

    & $Python -u -m fold_lm.v05_benchmarks.gate_e_c206_terminal_derived_result_integration --c205-summary $C205Summary --c204-summary $C204Summary --c203-summary $C203Summary --c202-summary $C202Summary --c201-summary $C201Summary --c200-summary $C200Summary --c199-summary $C199Summary --c174-summary $C174Summary --c181-summary $C181Summary --c188-summary $C188Summary --output-dir $Out --expected-head $ExpectedHead
    if($LASTEXITCODE -ne 0){throw "C206 execution failed; preserve invalid.json"}
...
print('acquisitions =',s['acquisitions'])
print('terminal_sufficient =',s['terminal_sufficient'])
print('verified_derived =',s['verified_derived'])
print('opposite_rejected =',s['opposite_rejected'])
print('verifier_calls =',s['verifier_calls'])
print('channel_counts =',s['channel_counts'])
print('channel_switches =',s['channel_switches'])
print('support_min =',s['support_min'])
print('support_max =',s['support_max'])
print('failures =',s['failures'])
print('projection_errors =',s['projection_errors'])
print('candidate_gate_passed =',b.gate(p['block_records'],s))
'@
    & $Python -u -c $Postcheck $Out $C205Summary $C204Summary $C203Summary $C202Summary $C201Summary $C200Summary $C199Summary $C174Summary $C181Summary $C188Summary $ExpectedHead
    if($LASTEXITCODE -ne 0){throw "C206 artifact postcheck failed"}
    $Completed=$true
}
finally{
    Write-Output "=== C206 POSTCHECK ==="
    Confirm-Repository
