...
if ($active.Count -ne 1 -or $active[0].Groups["id"].Value -ne "254") { Skip-Invocation "STALE_EXPERIMENT"; return }
$runnerPath = Join-Path $Root "tools\run_c254.ps1"
$runnerTokens = $null
...
try {
    & {
        Write-Output "=== C254 repository preflight ==="
        Write-Output "execution_head = $headNow"
        .\tools\run_c254.ps1 @runArgs
    } *>&1 | Tee-Object -FilePath $log
}
catch { $failure = $_ }
finally {
    try { .\tools\publish_experiment_log.ps1 -ExperimentId C254 -LogPath $log -ExecutionHead $ExpectedHead }
    catch {
        if ($null -ne $failure) { throw "C254 execution and publication failed: $($failure.Exception.Message); $($_.Exception.Message)" }
        throw
    }
}
