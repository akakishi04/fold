...
if ($active.Count -ne 1 -or $active[0].Groups["id"].Value -ne "244") { Skip-Invocation "STALE_EXPERIMENT"; return }
$runnerPath = Join-Path $Root "tools\run_c244.ps1"
$runnerTokens = $null
...
$runArgs = @{
    ...
    C243Summary = (Join-Path $Root "runs\c243-v5b-saved-recombination-audit-9a4a52f99ee2402dbbc3cb303935217a\summary.json")
    C242Summary = (Join-Path $Root "runs\c242-v5b-balanced-recombination-530e801ba23f496aa8e0a588e0d52982\summary.json")
}
$failure = $null
try {
    & {
        Write-Output "=== C244 repository preflight ==="
        Write-Output "execution_head = $headNow"
        .\tools\run_c244.ps1 @runArgs
    } *>&1 | Tee-Object -FilePath $log
}
catch { $failure = $_ }
finally {
    try { .\tools\publish_experiment_log.ps1 -ExperimentId C244 -LogPath $log -ExecutionHead $ExpectedHead }
    catch {
        if ($null -ne $failure) { throw "C244 execution and publication failed: $($failure.Exception.Message); $($_.Exception.Message)" }
        throw
    }
}
