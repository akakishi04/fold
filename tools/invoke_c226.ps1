...
if ($active.Count -ne 1 -or $active[0].Groups["id"].Value -ne "226") { Skip-Invocation "STALE_EXPERIMENT"; return }
$runnerPath = Join-Path $Root "tools\run_c226.ps1"
$runnerTokens = $null
...
$runArgs = @{
    ...
    C225Summary = (Join-Path $Root "runs\c225-v5f-stateful-baseline-4df0cd59e7c34942a3d33b0932863b0f\summary.json")
}
$failure = $null
try {
    & {
        Write-Output "=== C226 repository preflight ==="
        Write-Output "execution_head = $headNow"
        .\tools\run_c226.ps1 @runArgs
    } *>&1 | Tee-Object -FilePath $log
}
catch { $failure = $_ }
finally {
    try { .\tools\publish_experiment_log.ps1 -ExperimentId C226 -LogPath $log -ExecutionHead $ExpectedHead }
    catch {
        if ($null -ne $failure) { throw "C226 execution and publication failed: $($failure.Exception.Message); $($_.Exception.Message)" }
        throw
    }
}
