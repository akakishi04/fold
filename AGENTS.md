...
- `C###`実験の会話・実行指示・結果判定・別チャットへの引き継ぎは`docs/experiment-conversation-handoff-protocol.md`の**Current response format (v2)**を標準とする。accepted resultはformal verdict→validity→deciding metrics→interpretation→confound→repository update→同じ返答内で次Cのone-question設計/preregistration/launcherまで進める。INVALIDはsame-C execution recoveryだけを行い次Cへ進まない。ユーザー向け実行は原則`tools/invoke_c###.ps1`の短いlauncher形式、ログはremoteの`docs/experiment-run-logs/c###/latest.*`を取得して判定する。scientific execution HEADとpublished log commitを混同しない。
