...
def evaluate_case(db_path: Path, receipt_id: str, workers: int, *, write_current: bool):
    setup = SqliteRecoveryFencingStore(db_path)
    old_token = setup.acquire(receipt_id, "worker-old", now=0, lease_ticks=LEASE_TICKS)
    stores = [SqliteRecoveryFencingStore(db_path) for _ in range(workers)]
    barrier = threading.Barrier(workers)

    def takeover(index: int):
        barrier.wait()
        token = stores[index].acquire(receipt_id, f"worker-{index}", now=TAKEOVER_AT, lease_ticks=LEASE_TICKS)
        return index, token
    ...
    def write(index: int):
        ...
        if write_current and index == winner_index:
            return "current", stores[index].apply_fenced_write(
                receipt_id,
                f"worker-{index}",
                new_token,
                now=WRITE_AT,
        ...
        return "stale", stores[index].apply_fenced_write(
            receipt_id,
            "worker-old",
            old_token,
            now=WRITE_AT,
    ...
    effect = setup.read_effect(receipt_id)
