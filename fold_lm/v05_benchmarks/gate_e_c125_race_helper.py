    ...
    for visible, actual in c116._pairs():
        for hidden in (0, 1):
            if not any(actual):
                ...
                rows.append({"visible":list(visible),"actual":list(actual),"hidden":hidden,"outcome":"NONE_AVAILABLE","workers":0,"claims":0,"post_duplicate_rejected":True,"snapshot_count":0,"commit":0,"retry":0,"fallback":0,"final":"UNRESOLVED","trace":control["trace"],"passed":bool(control["passed"])})
                continue
            for outcome in c119h.OUTCOMES:
                for workers in WORKERS:
                    ...
                    rows.append({"visible":list(visible),"actual":list(actual),"hidden":hidden,"outcome":outcome,"workers":workers,"claims":claims,"post_duplicate_rejected":not duplicate_after,"snapshot_count":len(snapshot),"commit":control["commit"] if claims == 1 else 0,"retry":control["retry"] if claims == 1 else 0,"fallback":control["fallback"] if claims == 1 else 0,"final":control["final"],"trace":control["trace"],"passed":passed})
    return rows
