"""Exact-distance synthetic generator for FOLD-R long-memory capacity sweeps.

This keeps the original v1 ownership benchmark intact.  It emits the same
prompt/target schema, but inserts owner-independent neutral filler so the byte
distance from the distant fact owner to the answer owner is exactly the requested
value for every example.

Example::

    python -m fold_lm.long_memory_distance generate \
        --distance 256 \
        --output data/raw/long-memory-distance-256
"""
...
FILLER_UNIT = "Background detail remained unchanged. "


def _neutral_filler(length: int) -> str:
    """Return exactly ``length`` ASCII bytes of owner-independent filler."""
    if length < 0:
        raise ValueError("filler length must be nonnegative")
    unit = FILLER_UNIT
    whole, remainder = divmod(length, len(unit))
    return unit * whole + " " * remainder


def _build_exact_row(
    rng: random.Random,
    *,
    target_distance: int,
    local_window: int,
    ...
    filler = _neutral_filler(filler_bytes)
...
def generate_exact_distance_dataset(
    output: Path,
    *,
    target_distance: int,
    ...
    """Generate deterministic SFT JSONL at one exact fact-to-answer distance."""
    ...
    stage = Path(tempfile.mkdtemp(prefix=".long-memory-distance-", dir=output.parent))
    ...
    try:
        ...
        for _ in range(train_rows):
            row, filler_bytes, total_bytes = _build_exact_row(
                train_rng,
        ...
        meta = {
            "schema": 1,
            "benchmark": "fold-r-long-memory-owner-distance-v1",
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="FOLD-R exact-distance long-memory dataset generator")
    sub = parser.add_subparsers(dest="command", required=True)
    generate = sub.add_parser("generate", help="generate one exact-distance ownership dataset")
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = generate_exact_distance_dataset(
        args.output,
